"""
Compute Distracting Scores for Question-Passage Pairs

This script computes the "distracting score" (NO-RESPONSE probability) for each passage
in a dataset. The distracting score measures how likely a language model is to abstain
...
Input format:
  ...
  - "passages": list of passage objects, each with:
    ...
    - "is_relevant": boolean indicating if passage is relevant to the question
    - other optional fields (doc_id, title, etc.)

Output format:
  Same structure as input, but each passage gets additional fields:
  - "no_res_prob_{model_key}": the distracting score for that passage
"""
...
def compute_no_response_probability(
    ...
    """
    Compute the probability of generating 'NO-RESPONSE' token (distracting score).

    This score indicates how likely the model is to abstain from answering based on
    the given context. Higher scores mean the passage is more likely to be distracting
    or irrelevant.
    """
    # Tokenize the prompt
    ...
    with torch.no_grad():
        ...
        # Get probability of "NO-RESPONSE" token (distracting score)
...
def process_dataset(
    ...
    """Process dataset and add distracting scores for each passage."""
    results = []
    ...
    for item_idx, item in enumerate(dataset):
        ...
        for passage_idx, passage in enumerate(passages):
            try:
                ...
                distracting_score = compute_no_response_probability(model, tokenizer, prompt)

                # Add the distracting score
                score_key = f"no_res_prob_{model_key}"
                result_item['passages'][passage_idx][score_key] = distracting_score

                print(f"  Passage {passage_idx}: {distracting_score:.4f}")

            except Exception as e:
                print(f"  Error processing passage {passage_idx}: {e}")
                score_key = f"no_res_prob_{model_key}"
                result_item['passages'][passage_idx][score_key] = None
                continue
...
def main():
    ...
    parser.add_argument(
        ...
        help="Short key to identify model in output fields (e.g., llama-3.2-3b)"
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    # Process dataset
    results = process_dataset(model, tokenizer, dataset, args.model_key)
